...
static int path_for_file(lua_State *L) {
	...
	jmethodID activity_get_no_backup_dir = env->GetMethodID(activity_class, "getNoBackupFilesDir", "()Ljava/io/File;");
	...
	switch (directory) {
		...
		case system_directory_documents:
			...
			break;
		case system_directory_no_backup:
			{
				jobject files_file_object = env->CallObjectMethod(dmGraphics::GetNativeAndroidActivity(), activity_get_no_backup_dir);
				directory_path_string = (jstring)env->CallObjectMethod(files_file_object, file_get_canonical_path);
			}
			break;
		default:
...
dmExtension::Result INITIALIZE(dmExtension::Params *params) {
	...
	lua_pushnumber(L, system_directory_no_backup);
	lua_setfield(L, -2, "no_backup");
